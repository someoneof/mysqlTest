...
test() ->
    % compile:file("/usr/loc/al/lib/erlang/lib/mysql/mysql.erl"),
    ...
    mysql:start_link(p1, "localhost", "root", "haifeng", "test"),

    %% Add 2 more connections to the connection pool
    mysql:connect(p1, "localhost", undefined, "root", "haifeng", "test",
		  true),
    % mysql:connect(p1, "localhost", undefined, "root", "haifeng", "test",
		  % true),
    ...
    {data, Result1} = mysql:fetch(p1, <<"SELECT * FROM developer WHERE name = 'Ulf Wiger'">>),
   

    Rows = mysql:get_result_rows(Result1),
    rows_print(Rows),
